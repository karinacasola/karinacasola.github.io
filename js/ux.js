...
createApp({
    setup() {
        ...
        // --- Banco de Questões (As 100 Originais de Ecossistema UX) ---
        const questions = ref([
        ...
        const loadQuestion = async () => {
            isTyping.value = true;
            await typeWriter(`Carregando Cenário Aplicado ${currentQuestion.value.id}...`, "log-info");
        ...
        const nextQuestion = () => {
            if (currentQuestionIndex.value < questions.value.length - 1) {
                ...
                gameOver.value = true;
                addLog("Simulação concluída. Processando métricas de performance para exportação PDF...", "log-info");
            }
        };
        ...
        const selectOption = (option) => {
            ...
            if (option === currentQuestion.value.answer) {
                ...
                feedbackMsg.value = "<i class='bi bi-check-lg'></i> Decisão Correta! Projeto validado e UX otimizada.";
                addLog("Sucesso: Decisão metodológica acatada pelo comitê.", "log-success");
                ...
                if (attempts.value >= maxAttempts) {
                    ...
                    addLog("Falha Crítica de UX: Abandono de carrinho detectado nos logs.", "log-error");
                    showAnswer.value = true;
                    ...
                    addLog(`Aviso: O usuário ficou frustrado e contestou a interface. Tentativa ${attempts.value}/${maxAttempts}`, "log-warning");
                }
            }
        };
        ...
        const saveResultPDF = () => {
            ...
            if (score.value < 70) performanceMsg = "Recomenda-se revisão aprofundada das Leis de UX e Heurísticas.";
            
            // Alterado para o azul da identidade visual do blog (#3e8eff)
            printElement.innerHTML = `
                <div style="text-align: center; border-bottom: 2px solid #3e8eff; padding-bottom: 20px; margin-bottom: 30px;">
                    <h1 style="color: #3e8eff; margin: 0;">Relatório de Avaliação Heurística e UX</h1>
                ...
                <div style="margin-bottom: 30px; font-size: 16px; line-height: 1.6; text-align: justify;">
                    ...
                    <p>Este documento atesta a passagem do profissional pelas ${questions.value.length} análises críticas em cenários envolvendo wireframes, card sorting, acessibilidade visual, heurísticas de Nielsen e Bastien, e estruturação de projetos.</p>
                ...
                <p style="font-size: 13px; color: #888; text-align: center; margin-top: 50px; border-top: 1px dashed #ccc; padding-top: 15px;">
                    Documento gerado pelo Módulo UX.ESCAPE 1
                </p>
            `;
        ...
        const resetGame = () => {
            ...
            resetTurn();
            addLog("Reconfigurando parâmetros do simulador de UX...", "log-info");
            setTimeout(() => loadQuestion(), 1000);
        };

        onMounted(() => {
            addLog("Inicializando Simulador UX_360_v1.0 (PRO)...", "log-info");
            setTimeout(() => { loadQuestion(); }, 1000);
        });
