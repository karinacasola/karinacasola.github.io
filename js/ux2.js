...
createApp({
  data() {
    return {
      ...
      questions: [
        ...
        { id: 90, instruction: "Estratégia UX: Valor para o Usuário.", scenario: "Ao justificar uma nova funcionalidade, o PM pergunta o ganho do usuário.", text: "Qual o foco da Proposta de Valor em UX para o usuário?", options: ["Aumentar o atrito para mantê-lo mais tempo no site", "Deixar o site cheio de animações pesadas", "Resolve problemas reais, reduz atrito e economiza tempo", "Obrigar o preenchimento de mais formulários"], answer: "Resolve problemas reais, reduz atrito e economiza tempo" },
  ...
  mounted() {
    this.addLog("Inicializando Simulador Produto_e_AI_v2.0...", "log-info");
  ...
  methods: {
    selectOption(option) {
      ...
      if (option === this.currentQuestion.answer) {
        ...
        this.feedbackMsg = "Resposta Correta! Fluxo validado com sucesso.";
        this.addLog("Sucesso: Decisão arquitetural validada no projeto.", "log-success");
        ...
        if (this.attempts >= 3) {
          ...
          this.feedbackMsg = `Tentativas esgotadas. A resposta baseada no material: ${this.currentQuestion.answer}`;
          this.addLog("Falha Crítica: Decisão refutada na revisão de design.", "log-error");
          this.showAnswer = true;
          ...
          this.addLog(`Aviso: Revisão necessária. Tentativa ${this.attempts}/3`, "log-error");
        }
      }
    },
    nextQuestion() {
    ...
    async loadQuestion() {
      this.isTyping = true;
      await this.typeWriter(`Carregando Etapa de Projeto ${this.currentQuestion.id}...`, "log-info");
      await this.typeWriter(this.currentQuestion.scenario, "log-default");
      this.isTyping = false;
    },
    finishGame() {
      this.gameOver = true;
      this.addLog("Auditoria concluída. Processando resultados para emissão de certificado PDF...", "log-info");
    },
    ...
    saveResultPDF() {
      ...
      if (this.score < 70) performanceMsg = "Recomenda-se revisão sobre fundamentos de UX/UI.";
      
      printElement.innerHTML = `
        <div style="text-align: center; border-bottom: 2px solid #9c6bd4; padding-bottom: 20px; margin-bottom: 30px;">
            <h1 style="color: #9c6bd4; margin: 0;">Certificado de Auditoria de Produto Web</h1>
            <h2 style="color: #555; margin: 5px 0;">Avaliação Completa: UX, Estratégia e UI</h2>
        </div>
        <div style="margin-bottom: 30px; font-size: 16px; line-height: 1.6; text-align: justify;">
            <p><strong>Data da Auditoria:</strong> ${data}</p>
            <p><strong>Certificação:</strong> Especialista UX/UI</p>
            <p>Este documento atesta a passagem do profissional pelas ${this.questions.length} análises críticas envolvendo processos de Estratégia, Arquitetura de Informação, Guias de Estilo, Tipografia, Teoria das Cores e Ecossistema Figma.</p>
        ...
        <p style="font-size: 13px; color: #888; text-align: center; margin-top: 50px; border-top: 1px dashed #ccc; padding-top: 15px;">
            Documento validado tecnicamente pelo Simulador UX.ESCAPE 2
        </p>
      `;

      const opt = {
        margin:       0.5,
        filename:     'UX_Certificacao_Completa.pdf',
    ...
    resetGame() {
      ...
      this.resetTurn();
      this.addLog("Reconfigurando parâmetros do simulador de Produto Web...", "log-info");
      setTimeout(() => this.loadQuestion(), 1000);
    }
  }
}).mount('#app');